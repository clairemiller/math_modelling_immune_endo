...
subroutine FUNC(ndim, u, icp, par, ijac, f, dfdu, dfdp)
    ...
    DOUBLE PRECISION m2_upreg ! temporary while we investigate
    ...
    ! m2_upreg = 1.0D0 + EA / (CEA + EA)
    m2_upreg = 1.0D0 + (1.0e-1)*EA

    ! System equations ---------------------------------------------------------------------
    f(1) = muM_scaled + etaM*(1.0D0-(M0+M1+M2))*(M0+M1+M2)-beta1_scaled*(EF+EA)*M0-thetaM*(KA/(CKA_scaled+KA))*M0-beta2*M0*m2_upreg-deltaM*M0
    f(2) = beta1_scaled*(EF+EA)*M0+thetaM*(KA/(CKA_scaled+KA))*M0+beta21*M2-beta12*M1-deltaM*M1                           
    f(3) = beta2 * M0 * m2_upreg + beta12 * M1 - beta21 * M2 - deltaM * M2                                                                  
