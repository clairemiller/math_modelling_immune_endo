...
subroutine FUNC(ndim, u, icp, par, ijac, f, dfdu, dfdp)
    ...
    ! System equations ---------------------------------------------------------------------
    f(1) = signed_system*(muM_scaled + etaM*(1.0D0-(M0+M1+M2))*(M0+M1+M2)-beta1_scaled*(EF+EA)*M0-thetaM*(KA/(CKA_scaled+KA))*M0-beta2*M0-deltaM*M0)
    f(2) = signed_system*(beta1_scaled*(EF+EA)*M0+thetaM*(KA/(CKA_scaled+KA))*M0+beta21*M2-beta12*M1-deltaM*M1)                           
    f(3) = signed_system*(beta2 * M0 + beta12 * M1 - beta21 * M2 - deltaM * M2)                                                                  
...
subroutine STPNT(ndim, u, par, t)
    ...
    ! u(1) = signed_system*0.0025987 ! M0
    ! u(2) = signed_system*0.9474 ! M1
    ! u(3) = signed_system*0.0024922 ! M2 
    ! u(4) = signed_system*0.0053585 ! K0
    ! u(5) = signed_system*1.4678e-06! KA 
    ! u(6) = signed_system*1.26e-6   ! E0
    ! u(7) = signed_system*1.5014e-07   ! EF
    ! u(8) = signed_system*0.23115 ! EA

    par(1) = -6     ! Initial value for beta1_scaled
    par(2) = -5    ! Initial value for omega_scaled
  
    u(1) = signed_system*0.90443 ! M0
    u(2) = signed_system*0.0029523 ! M1
    u(3) = signed_system*0.045116 ! M2 
