...
subroutine FUNC(ndim, u, icp, par, ijac, f, dfdu, dfdp)
    ...
    DOUBLE PRECISION m2_upreg, alpha, omega_scaled ! Parameters of interest
    
    M0 = signed_system*u(1)
    M1 = signed_system*u(2)
    M2 = signed_system*u(3)
    K0 = signed_system*u(4)
    KA = signed_system*u(5)
    E0 = signed_system*u(6)
    EF = signed_system*u(7)
    EA = signed_system*u(8)
    ...
    ! System equations ---------------------------------------------------------------------
    f(1) = signed_system*(muM_scaled + etaM*(1.0D0-(M0+M1+M2))*(M0+M1+M2)-beta1_scaled*(EF+EA)*M0-thetaM*(KA/(CKA_scaled+KA))*M0-beta2*M0*m2_upreg-deltaM*M0)
    f(2) = signed_system*(beta1_scaled*(EF+EA)*M0+thetaM*(KA/(CKA_scaled+KA))*M0+beta21*M2-beta12*M1-deltaM*M1)                           
    f(3) = signed_system*(beta2 * M0 * m2_upreg + beta12 * M1 - beta21 * M2 - deltaM * M2)                                                                  
    f(4) = signed_system*(muK_scaled-thetaK*K0*(M1/(CM1_scaled+M1))-deltaK*K0)                                                                   
    f(5) = signed_system*(thetaK*K0*(M1/(CM1_scaled+M1))+etaK*(1.0D0-(KA+K0))*KA-sigma_scaled*(EF+EA)*KA-deltaK*KA)                              
    f(6) = signed_system*(muE_scaled-deltaE0*(rho0*E0+(1-rho0)*E0))                                                                       
    f(7) = signed_system*(deltaE0*rho0*E0-omega_scaled*(gamma*KA+(1.0D0-gamma)*M1)*EF-rhoF*EF-deltaE*EF)                            
    f(8) = signed_system*(rhoF*EF+etaE*(1.0D0-EA)*(M2/(CM2_scaled+M2))*EA-omega_scaled*(gamma*KA+(1.0D0-gamma)*M1)*EA-deltaE*EA)

    IF(ijac.EQ.0)RETURN
  
    ! The jacobian -------------------------------------------------------------------------
    ! M0
    dfdu(1,1) = -1.0D0 * beta1_scaled*(EA + EF) - beta2 * m2_upreg - deltaM + etaM*(1.0D0 - 2.0D0*(M0 + M1 + M2)) - (thetaM * KA)/(CKA_scaled + KA) !! modified with M2 upregulation
    ...
    dfdu(1,8) = -1.0D0 * beta1_scaled * M0 - CEA_scaled*M0*beta2*alpha/((EA+CEA_scaled)*(EA+CEA_scaled)) !! modified with M2 upregulation
    ! M1
    ...
    ! M2
    dfdu(3,1) = beta2 * m2_upreg !! modified with M2 upregulation
    dfdu(3,2) = beta12
    ...
    dfdu(3,6) = 0.0D0
    dfdu(3,7) = 0.0D0
    dfdu(3,8) = CEA_scaled*M0*beta2*alpha/((EA+CEA_scaled)*(EA+CEA_scaled)) !! modified with M2 upregulation
    ! K0
    dfdu(4,1) = 0.0D0
...
subroutine STPNT(ndim, u, par, t)
    ...
    par(1) = 5.0D0      ! Initial value for alpha (logscale)
    par(2) = -5.0D0     ! Initial value for omega_scaled
  
    u(1) = signed_system*0.00011179 ! M0
    u(2) = signed_system*0.39463 ! M1
    u(3) = signed_system*0.55775 ! M2 
    u(4) = signed_system*0.0060503 ! K0
    u(5) = signed_system*4.7908e-06 ! KA 
    u(6) = signed_system*1.26e-6   ! E0
    u(7) = signed_system*1.2241e-07   ! EF
    u(8) = signed_system*0.07053 ! EA
    ...
    ! u(1) = signed_system*0.90442 ! M0
    ! u(2) = signed_system*0.0029523 ! M1
    ! u(3) = signed_system*0.045118 ! M2 
    ! u(4) = signed_system*0.08734 ! K0
    ! u(5) = signed_system*0.24258 ! KA 
    ! u(6) = signed_system*1.26e-6   ! E0
    ! u(7) = signed_system*5.7626e-08   ! EF
    ! u(8) = signed_system*5.293e-09 ! EA
    ...
    ! u(1) = signed_system*2.9035e-05 ! M0
    ! u(2) = signed_system*0.95009 ! M1
    ! u(3) = signed_system*0.0023722 ! M2 
    ! u(4) = signed_system*0.0053571 ! K0
    ! u(5) = signed_system*5.1715e-07 ! KA 
    ! u(6) = signed_system*1.26e-06   ! E0
    ! u(7) = signed_system*2.9301e-07   ! EF
    ! u(8) = signed_system*0.65607 ! EA
    ...
    ! u(1) = signed_system*0.90678 ! M0
    ! u(2) = signed_system*0.00023378 ! M1
    ! u(3) = signed_system*0.045482 ! M2 
    ! u(4) = signed_system*0.16179 ! K0
    ! u(5) = signed_system*0.059366 ! KA 
    ! u(6) = signed_system*1.26e-06   ! E0
    ! u(7) = signed_system*2.6371e-09   ! EF
    ! u(8) = signed_system*5.6491e-12 ! EA
